class stageGeometry {
  constructor(stage, stageNum, loadData, debugMode) {
    this.stage = stage;
    ...
    this.para = {
      star: {
        txtObj: new createjs.Text(
          `0 / ${Object.keys(stageGeometryArray[stageNum].star).length}`,
          "24px Arial",
          "black",
        ),
        paraShowName: "星",
      ...
      seconds: {
        ...
        titleImg: (loadData) => {
          const timerIcon = new createjs.Bitmap(
            loadData.target._loadedResults.timer,
          );
      ...
      position: {
        ...
        paraShowName: "座標",
        titleImg: (loadData) => {
          const exeicon = new createjs.Bitmap(
            loadData.target._loadedResults.science,
          );
    ...
    const positionReload = () => {
      ...
      clearCondition.forEach((oneCondition) => {
        let parameterA = String(this.para[oneCondition[0]].txtObj.text)
          .split("/")[0]
          .replace(" ", "");
  ...
  pushChank(obj, objInfo, type) {
    ...
    const add = (num) => {
      ...
      if (this.chankBlock[String(num)][type] == undefined)
        this.chankBlock[String(num)][type] = [];
  ...
  makeStar() {
    ...
    starPosiList.forEach((starPosi) => {
      ...
      const fillCommand =
        oneStar.graphics.beginFill("rgb(255, 191, 94)").command;
  ...
  makeCustomElem() {
    ...
    custuemElementInfo.forEach((oneElem) => {
      ...
      if (type == "event") {
        this.chankBlock.custom.push({
          name: oneElem.name,
          customEvent: oneElem.customEvent,
        });
        oneElem.start();
      }
  ...
  drawParameter() {
    ...
    this.paraDictionary.forEach((paraName, index) => {
      ...
      paraComp.visible = oneParaInfo.visible;
      paraComp.y = 10;
      paraComp.x = nowX + 20;
