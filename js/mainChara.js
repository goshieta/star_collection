...
class oneMainChara {
  ...
  movechara() {
    ...
    if (
      Math.floor((this.position[0] + 31) / 100) * 100 ==
      Math.floor((this.position[0] - 31) / 100) * 100
    )
      searchStarArr = searchStarArr.concat(returnArray(this.position[0]).star);
    else
      searchStarArr = Array.from(
        new Set(
          searchStarArr
            .concat(returnArray(this.position[0] + 31).star)
            .concat(returnArray(this.position[0] - 31).star),
        ),
      );
    //検査
    searchStarArr.forEach((star, index) => {
      ...
      if (this.mainChara.hitTest(point.x, point.y)) {
        ...
        createjs.Sound.play("collectedStar");
        this.haveStar++;
        this.parameter["star"].txtObj.text =
          `${this.haveStar} / ${this.startStarLength}`;
      }
    });
    ...
    this.chankBlock.custom.forEach((oneCustom) => {
      ...
      if (
        ...
        searchCeArr = searchCeArr.concat(
          returnArray(this.position[0])[oneCustom.name],
        );
      else
        searchCeArr = Array.from(
          new Set(
            searchCeArr
              .concat(returnArray(this.position[0] + 31)[oneCustom.name])
              .concat(returnArray(this.position[0] - 31)[oneCustom.name]),
          ),
        );
    ...
    if (this.startTime != null) {
      ...
      let nowH =
        this.firstSpeed * startTimeRange - (9.8 * startTimeRange ** 2) / 2;
      if (nowBooltouchGround) {
        ...
        if (HeightDiff > 0) {
          ...
          let searchArr = Array.from(
            new Set(
              returnArray(this.position[0] + 31)["ground"].concat(
                returnArray(this.position[0] - 31)["ground"],
              ),
            ),
          );
          searchArr.forEach((og) => {
            ...
            if (
              thisTimeGroundY - thisTimeY >= 0 &&
              (mostNearY > thisTimeGroundY || mostNearY == undefined)
            ) {
    ...
    if (this.position[1] < -2500) {
      this.bgStage.endGameStatus = true;
